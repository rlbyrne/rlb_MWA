pro tukey_test_scatter_plot_masking, $
    savefile = savefile, $
  ...
  if keyword_set(tukey) then begin
    note = 'tukey filter, pol ' + polarizations + ', term ' + strtrim(string(choose_terms),2)
  endif else begin
    note = 'no tukey filter, pol ' + polarizations + ', term ' + strtrim(string(choose_terms),2)
  endelse
  
  wt_meas_min_clip = 0.1
  ...
  for term = 0, n_elements(choose_terms) - 1 do begin
    for pol = 0, n_elements(polarizations) - 1 do begin
      ...
      if keyword_set(tukey) then begin
        output_loc = '/nfs/eor-00/h1/rbyrne/error_analysis_plots/varcompare_masked_tukey_'+use_cube+'_'+polarizations[pol]+'_term' + STRTRIM(STRING(choose_terms[term]), 2)
      endif else begin
        output_loc = '/nfs/eor-00/h1/rbyrne/error_analysis_plots/varcompare_masked_tophat_'+use_cube+'_'+polarizations[pol]+'_term' + STRTRIM(STRING(choose_terms[term]), 2)
      endelse
      ...
      CROSS_DIFFERENCE, choose_term = choose_terms[term], filename1 = filename1, filename2 = filename2, /diff_sim,$
        ...
        uv_img_clip, save_cubes = 0
        
      ;; PLOT SIMULATION:
        
      sigma2_kperp_all = MEAN(sigma2_all, DIMENSION = 3, /NAN)
      var_diff_cross_sim = VARIANCE(REAL_PART(diff_cross_sim), DIMENSION = 3)
      keep_indices = WHERE(sigma2_kperp_all GT 0 AND var_diff_cross_sim GT 0 AND wt_meas_min_1 ge wt_meas_min_clip and wt_meas_min_2 ge wt_meas_min_clip)
      ...
      hist_vals_sim = FLOAT(hist_vals_sim) / TOTAL(hist_vals_sim) ;normalize the histogram
      ...
      QUICK_IMAGE, hist_vals_sim, sigma2_locs, var_diff_locs, /XLOG, /YLOG, XRANGE = xrange, YRANGE = yrange,  $
        DATA_ASPECT = 1, DATA_RANGE = data_range, TITLE = 'Gaussian Model, all kperp', $
      ...
      FOR i = 0, N_ELEMENTS(kperp_wavelength_max) - 1 DO BEGIN
        ...
        IF kperp_wavelength_max[i] NE 0 THEN BEGIN
          kx_indices = WHERE(ABS(kx_mpc) LE kperp_wavelength_max[i]/kperp_lambda_conv AND ABS(kx_mpc) GE kperp_wavelength_min[i]/kperp_lambda_conv, /NULL)
          ky_indices = WHERE(ABS(ky_mpc) LE kperp_wavelength_max[i]/kperp_lambda_conv AND ABS(kx_mpc) GE kperp_wavelength_min[i]/kperp_lambda_conv, /NULL)
        ENDIF ELSE BEGIN
          kx_indices = WHERE(ABS(kx_mpc) GE kperp_wavelength_min[i]/kperp_lambda_conv, /NULL)
          ky_indices = WHERE(ABS(ky_mpc) GE kperp_wavelength_min[i]/kperp_lambda_conv, /NULL)
        ENDELSE
        
        sigma2_kperp_use = sigma2_kperp_all[kx_indices, ky_indices, *]
        var_diff_cross_use = var_diff_cross[kx_indices, ky_indices, *]
        wt_meas_min_1 = wt_meas_min_1[kx_indices, ky_indices,*]
        wt_meas_min_2 = wt_meas_min_2[kx_indices, ky_indices,*]
        keep_indices = WHERE(sigma2_kperp_use GT 0 AND var_diff_cross_use GT 0 AND wt_meas_min_1 ge wt_meas_min_clip and wt_meas_min_2 ge wt_meas_min_clip)
        ...
        hist_vals = HIST_2D(sigma2_kperp_log, var_diff_cross_log, MIN1 = min_sigma2, MAX1 = max_sigma2, MIN2 = min_var_diff, MAX2 = max_var_diff,$
          BIN1 = bin_sigma2, BIN2 = bin_var_diff)
        hist_vals = FLOAT(hist_vals) / TOTAL(hist_vals) ;normalize the histogram
