; Script to convert catalogs to FHD formats if structure conflicts exist
; Written by Ruby Byrne, 12/18

pro catalog_reformat

  filepath = '/Users/rubybyrne/FHD/catalog_data/GLEAM_v2_plus_rlb2019.sav'
  ...
  struct_base={id:-1L,x:0.,y:0.,ra:0.,dec:0.,ston:0.,freq:180.,alpha:0.,gain:1.,flag:0,extend:Ptr_new(),flux:flux_struct}
  
  catalog_old = getvar_savefile(filepath, 'catalog')
  
  catalog_new = Replicate(struct_base,n_elements(catalog_old)>1)
  for source = 0, n_elements(catalog_old)-1 do begin
    if catalog_old[source].shape.x gt 0. or catalog_old[source].shape.y gt 0. then stop
    catalog_new[source].ra = catalog_old[source].ra
    catalog_new[source].dec = catalog_old[source].dec
    catalog_new[source].freq = catalog_old[source].freq
  endfor
