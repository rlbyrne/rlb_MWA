...
class HealpixMap:
    def __init__(
        self, signal_arr, pix_arr, nside, nest=False, coords="equatorial", quiet=False
    ):
        if len(pix_arr) == 0:
            if not quiet:
    ...
    def get_ra_dec(self, ra_cut=270):
        if self.coords == "galactic":
            ...
            dec_arr = 90.0 - np.degrees(theta_eq)
        elif self.coords == "equatorial":
            ...
            sys.exit(1)
        ra_arr[np.where(ra_arr > ra_cut)] = ra_arr[np.where(ra_arr > ra_cut)] - 360.0
        ra_arr[np.where(ra_arr < ra_cut - 360.0)] = (
            ra_arr[np.where(ra_arr < ra_cut - 360.0)] + 360.0
        )
        self.ra_arr = ra_arr
    ...
    def get_galactic_coords(self):
        if self.coords == "galactic":
            ...
            theta_eq = np.radians(90.0 - dec_arr)
    ...
    def get_pixel_corners(self, ra_cut=270):
        corner_coords = hp.boundaries(self.nside, self.pix_arr, step=1, nest=self.nest)
        if self.coords == "galactic":
            ...
            for pixel in range(len(self.signal_arr)):
                ...
                if len(where_above[0]) > 0:
                    ras_pix[where_above] = ras_pix[where_above] - 360.0
                where_below = np.where(ras_pix < ra_cut - 360.0)
                if len(where_below[0]) > 0:
                    ras_pix[where_below] = ras_pix[where_below] + 360.0
                where_separated = np.where(ras_pix - np.amin(ras_pix) > 180.0)
                if len(where_separated[0]) > 0:
                    ras_pix[where_separated] = ras_pix[where_separated] - 360.0
                ras.append(ras_pix)
    ...
    def filter_map(self, lmin=None, lmax=None, filter_width=0, return_full_map=False):
        # This function borrows from code by Miguel Morales
        if lmax is not None:
            alm_limit = int(math.ceil(lmax + filter_width / 2.0))
        else:
            alm_limit = None
        ...
        if lmin is not None:
            for i in range(int(math.ceil(lmin - filter_width / 2.0))):
                window_fn[i] = 0
            for i in range(
                int(math.ceil(lmin - filter_width / 2.0)),
                int(math.floor(lmin + filter_width / 2.0)) + 1,
            ):
                window_fn[i] = (
                    -math.cos((i - lmin + filter_width / 2.0) * math.pi / filter_width)
                    + 1
                ) / 2.0
        if lmax is not None:
            for i in range(
                int(math.ceil(lmax - filter_width / 2.0)),
                int(math.floor(lmax + filter_width / 2.0)) + 1,
            ):
                window_fn[i] = (
                    math.cos((i - lmax + filter_width / 2.0) * math.pi / filter_width)
                    + 1
                ) / 2.0
            for i in range(
                int(math.floor(lmax + filter_width / 2.0)) + 1, len(window_fn)
            ):
                window_fn[i] = 0
        ...
        filtered_map = HealpixMap(
            hp.sphtfunc.alm2map(alm, self.nside),
            np.array([]),
            nside=self.nside,
            nest=False,
            coords=self.coords,
    ...
    def write_data_to_fits(self, save_filename):
        signal_column = fits.Column(
            name="SIGNAL", array=np.array(self.signal_arr), format="1E"
        )
        pixelnum_column = fits.Column(
            name="PIXEL", array=np.array(self.pix_arr), format="1J"
        )
        ...
        hdu_1 = fits.BinTableHDU.from_columns(
            [signal_column, pixelnum_column], header=header
        )
...
def load_maps_from_sav(data_filename, quiet=False):
    # Load a HEALPix map saved as an IDL sav file

    if not quiet:
        print("Loading HEALPix maps from {}".format(data_filename))
    contents = scipy.io.readsav(data_filename)
    nside = int(contents["nside"])
    ordering = "ring"
    coords = "equatorial"
    pixel_vals = contents["hpx_i_use"]
    signal_vals_q = contents["q_leakage"]
    signal_vals_u = contents["u_leakage"]
    ...
    healpix_map_q = HealpixMap(
        signal_vals_q, pixel_vals, nside, nest=nest, coords="equatorial"
    )
    healpix_map_u = HealpixMap(
        signal_vals_u, pixel_vals, nside, nest=nest, coords="equatorial"
    )
    return healpix_map_q, healpix_map_u
...
def load_global_map(data_filename):
    ...
    healpix_map = HealpixMap(signal_vals, [], nside, nest=nest, coords="galactic")
...
def load_fhd_input_map(data_filename, cube_ind=0):
    ...
    hpx_inds = data["hpx_inds"]
    hpx_inds.astype(int)
    healpix_map = HealpixMap(map_data, hpx_inds, nside, nest=False, coords="equatorial")
    return healpix_map
...
def difference_healpix_maps(map1, map2):
    ...
    if np.max(np.abs(map1.pix_arr - map2.pix_arr)) == 0:
        ...
        diff_map = HealpixMap(data_diff, map1.pix_arr, map1.nside, nest=map1.nest)
    else:
        ...
        diff_map = HealpixMap(data_diff, pixelnum_use, map1.nside, nest=map1.nest)
    return diff_map
...
def multiply_healpix_maps(map1, map2):
    ...
    if map1.pix_arr == map2.pix_arr:
        ...
        prod_map = HealpixMap(data_prod, map1.pix_arr, map1.nside, nest=map1.nest)
    else:
        pixelnum_use = list(set(map1.pix_arr).intersection(map2.pix_arr))
        data_prod = [
            map1.signal_arr[map1.pix_arr.index(pixelnum)]
        ...
        prod_map = HealpixMap(data_prod, pixelnum_use, map1.nside, nest=map1.nest)
    return prod_map
...
def divide_healpix_maps(map1, map2):
    ...
    pixelnum_use = pixelnum_use[np.where(denominator != 0.0)[0]]
    data_div = np.squeeze(
        np.array(
            [
    ...
    div_map = HealpixMap(data_div, pixelnum_use, map1.nside, nest=map1.nest)
    return div_map
...
def average_healpix_maps_simple(maps_arr):
    ...
    for map in maps_arr:
        ...
        if map.nest != nest:
            ...
            sys.exit(1)
    pixelnum_use = list(set([pixelnum for map in maps_arr for pixelnum in map.pix_arr]))
    pixel_vals = [[] for i in range(len(pixelnum_use))]
    for map in maps_arr:
        for i, pixelnum in enumerate(pixelnum_use):
            if pixelnum in map.pix_arr:
                pixel_vals[i].append(map.signal_arr[map.pix_arr.index(pixelnum)])

    ave_map = HealpixMap(
        [np.average(signal_vals) for signal_vals in pixel_vals],
        pixelnum_use,
        nside,
        nest=nest,
    )
    var_map = HealpixMap(
        [np.var(signal_vals) for signal_vals in pixel_vals],
        pixelnum_use,
        nside,
        nest=nest,
    )
    nsamples_map = HealpixMap(
        [len(signal_vals) for signal_vals in pixel_vals], pixelnum_use, nside, nest=nest
    )
...
def average_healpix_maps(
    ...
    for path_ind, fhd_run_path in enumerate(fhd_run_paths):
        ...
        if obs_list is None:  # use all obs in the data directory
            if restore_from_sav:
                print("ERROR: No obs_list provided. Exiting.")
                sys.exit(1)
            else:
                if obs_weights is not None:
                    print(
                        "WARNING: No obs_list provided. Disregarding obs_weights and using equal weighting."
                    )
                    obs_weights = None
        ...
        if not restore_from_sav:
            data_files = os.listdir("{}/output_data/".format(fhd_run_path))
            exclude_obs_list = []
            for obs in obs_list:
                for cube in cube_names:
                    if (
                        "{}_{}_{}_HEALPix.fits".format(obs, weighting, cube)
                        not in data_files
                    ):
                        print(
                            "WARNING: File {}/output_data/{}_{}_{}_HEALPix.fits not found. Excluding observation {} from the average.".format(
            ...
            obs_weights = np.array(
                [
                    obs_weights[ind]
                    for ind in range(len(obs_weights))
        ...
        if not quiet:
            print(
                "Averaging {} observations from {}".format(len(obs_list), fhd_run_path)
            )
        ...
        for obs_ind, obsid in enumerate(obs_list):
            ...
            maps = []
            if restore_from_sav:
                mapQ, mapU = load_maps_from_sav(
                    "{}/{}_hpx_pol_leakage_surface.sav".format(sav_file_path, obsid)
                )
                for cube_ind, map in enumerate([mapQ, mapU]):
                    if (
                        obs_ind == 0 and cube_ind == 0 and path_ind == 0
                    ):  # Use first map for conventions
                        if nside is None:
                            nside = map.nside
                            print(
                                "nside is not specified. Using nside {}.".format(nside)
                            )
                        else:
                            if map.nside != nside:
                                map.resample(nside)
                ...
                for cube_ind, cube in enumerate(cube_names):
                    map = load_map(
                        "{}/output_data/{}_{}_{}_HEALPix.fits".format(
            ...
            for pix in maps[0].pix_arr:  # Use the first cube for the pixel list
                if apply_radial_weighting:
                    ...
                    rad_weight = obs_radial_weighting_function(
                        hp.rotator.angdist(pix_vec, obs_vec) * 180.0 / np.pi
                    )
    ...
    weighted_ave_signal_array[:, np.where(weights_array == 0)] = hp.pixelfunc.UNSEEN

    averaged_maps = []
    for cube_ind in range(len(cube_names)):
        map = HealpixMap(
            weighted_ave_signal_array[cube_ind, :],
            [],
            nside,
            nest=nest,
            coords=coords,
...
def rm_correction(
    ...
    if use_single_freq_calc:
        ...
        new_q = (
            np.cos(2 * rot_angle) * maps[1].signal_arr
            + np.sin(2 * rot_angle) * maps[2].signal_arr
        )
        new_u = (
            -np.sin(2 * rot_angle) * maps[1].signal_arr
            + np.cos(2 * rot_angle) * maps[2].signal_arr
        )
    else:
...
def calculate_variance_healpix_maps(
    ...
    if saved_averaged_maps is None:
        ...
        if len(saved_averaged_maps) != len(cube_names):
            print(
                "WARNING: Invalid saved_averaged_maps paths provided. Recalculating data average."
            )
            averaged_maps, null = average_healpix_maps(
                fhd_run_paths,
                obs_lists=obs_lists,
    ...
    weighted_ave_var_array[:, np.where(nsamples_array == 1)] = 0

    variance_maps = []
    snr_maps = []
    for cube_ind in range(len(cube_names)):
        var_map = HealpixMap(
            np.squeeze(weighted_ave_var_array[cube_ind, :]),
            [],
            nside,
            nest=nest,
            coords=coords,
...
def obs_radial_weighting_function(
    ...
    if dist < max_dist - taper_width:
        ...
        weight = (
            (0.5 - zero_point)
            * np.cos(np.pi * (dist + taper_width - max_dist) / taper_width)
            + 0.5
            + zero_point
        )
    return weight
...
def combine_maps_nearest_data(
    ...
    for obs_ind, obsid in enumerate(obs_list):
        ...
        for cube_ind, cube in enumerate(cube_names):
            map = load_map(
                "{}/output_data/{}_uniform_{}_HEALPix.fits".format(
        ...
        unique_pix = np.setdiff1d(maps[0].pix_arr, overlapping_pix, assume_unique=True)
        pixels_used = np.append(pixels_used, unique_pix)

        for pix in overlapping_pix:
            vec = hp.pix2vec(nside, pix, nest=nest)
            distances = [
                (vec[0] - obs_centers[obs_ind][0]) ** 2.0
                + (vec[1] - obs_centers[obs_ind][1]) ** 2.0
                + (vec[2] - obs_centers[obs_ind][2]) ** 2.0
    ...
    for cube_ind in range(len(cube_names)):
        map = HealpixMap(
            signal_array[cube_ind, :], [], nside, nest=nest, coords=coords, quiet=True
        )
        map.implicit_to_explicit_ordering()
...
def write_data_to_standard_fits(maps, save_filename, history_str=""):
        ...
        if not np.array_equal(maps[map_ind].pix_arr, maps[0].pix_arr):
            ...
            for pix_ind in range(len(maps[map_ind].pix_arr)):
                new_signal_arr[pix_ind] = maps[map_ind].signal_arr[
                    np.where(maps[map_ind].pix_arr == maps[0].pix_arr[pix_ind])[0][0]
                ]
    ...
    header["CRVAL" + str(ax_nums["freq"])] = 182.0
    header["CRPIX" + str(ax_nums["freq"])] = 1
    header["CDELT" + str(ax_nums["freq"])] = 30.0

    # set up polarization axis
    header["CTYPE" + str(ax_nums["pol"])] = (
        "STOKES",
        "Polarization integers: I,Q,U,V=1,2,3,4",
    )
